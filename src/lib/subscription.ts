...
export function getUserSubscriptionInfo(user: User | UserResource | null): SubscriptionInfo {
  if (!user) {
    return {
      plan: 'guest',
      isActive: false,
  ...
  const privateMetadata = ('privateMetadata' in user ? user.privateMetadata : user.publicMetadata) as Record<string, unknown>
  ...
  return {
    ...
    stripeCustomerId: privateMetadata?.stripeCustomerId as string | undefined,
    subscriptionId: privateMetadata?.subscriptionId as string | undefined,
